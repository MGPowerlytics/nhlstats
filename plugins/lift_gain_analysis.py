...
def load_games_from_duckdb(sport: str, season_only: bool = False) -> pd.DataFrame:
    ...
    temp_db = Path(tempfile.gettempdir()) / f"nhlstats_temp_{datetime.now().timestamp()}.duckdb"
    try:
            ...
            for result_set in data['resultSets']:
                if result_set['name'] == 'GameHeader':
                    ...
                    for row in result_set['rowSet']:
                        ...
                        if all([home_team_name, away_team_name, home_score is not None, away_score is not None]):
                            games.append({
                                'game_id': game_id,
                                'game_date': game_date,
...
def calculate_elo_predictions(sport: str, games_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, game in df.iterrows():
        ...
        # Update ratings
        if sport in ['mlb', 'nfl']:
            elo.update(game['home_team'], game['away_team'], int(game['home_score']), int(game['away_score']))
        elif sport == 'epl':
             # EPL update needs result char ('H', 'D', 'A')
             result = game.get('result')
             if not result:
                 # Infer from binary if missing? But we should have it.
                 if game['home_score'] > game['away_score']: result = 'H'
...
def calculate_lift_gain_by_decile(df: pd.DataFrame, prob_col: str = 'elo_prob') -> pd.DataFrame:
    ...
    if len(df) < 10:
        print(f"⚠️  Not enough games for decile analysis (need at least 10, have {len(df)})")
        return pd.DataFrame()
    ...
    results_df['cumulative_win_rate'] = results_df['cumulative_wins'] / results_df['cumulative_games']
    results_df['gain_pct'] = (results_df['cumulative_wins'] / total_wins * 100) if total_wins > 0 else 0
...
def print_decile_table(decile_df: pd.DataFrame, sport: str, title: str):
    ...
    print(f"{'='*110}")
    print(f"Total Games: {total_games:,} | Total Home Wins: {total_wins:,} | Baseline Win Rate: {baseline:.1%}")
    print(f"{'='*110}")
    ...
    for _, row in decile_df.iterrows():
        ...
        print(f"{int(row['decile']):<8} {row['prob_range']:<16} {row['n_games']:<8} {row['n_wins']:<8} "
    ...
    if not bottom_deciles.empty:
        ...
        print(f"📉 BOTTOM 2 DECILES (1-2): {bot_games} games, {bot_wins} wins, "
...
def analyze_sport(sport: str, include_season: bool = True):
    ...
    if include_season:
        ...
        if not season_games.empty:
            ...
            print_decile_table(season_deciles, sport, f"CURRENT SEASON (Since {season_start})")
        else:
