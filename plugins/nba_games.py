        ...
        if "resultSets" in scoreboard:
            for result_set in scoreboard["resultSets"]:
                if result_set["name"] == "GameHeader":
                    ...
                    game_id_idx = (
                        headers.index("GAME_ID") if "GAME_ID" in headers else 2
                    )
        ...
        for i, game_id in enumerate(game_ids, 1):
            try:
                ...
                if not boxscore_file.exists():
                    print(
                        f"  [{i}/{len(game_ids)}] Downloading boxscore for game {game_id}"
                    )
                ...
                if not pbp_file.exists():
                    print(
                        f"  [{i}/{len(game_ids)}] Downloading play-by-play for game {game_id}"
                    )
                    ...
                    if playbyplay:
                        ...
                        actions_count = len(
                            playbyplay.get("game", {}).get("actions", [])
                        )
                        print(f"    ✓ Saved {actions_count} actions")
                else:
                    print(
                        f"  [{i}/{len(game_ids)}] Play-by-play exists for game {game_id}"
                    )
