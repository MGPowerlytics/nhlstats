...
class DBManager:
    ...
    def __init__(self, connection_string: Optional[str] = None, schema: Optional[str] = None):
        ...
        if connection_string:
            ...
            self.connection_string = f"postgresql+psycopg2://{user}:{password}@{host}:{port}/{db}"
    ...
    def fetch_df(self, query: str, params: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"Error fetching DataFrame: {query}\nParams: {params}\nError: {e}")
            raise

    def insert_df(self, df: pd.DataFrame, table_name: str, if_exists: str = 'append', index: bool = False):
        """
        Insert a DataFrame into a table.
        """
        try:
            df.to_sql(table_name, self.engine, if_exists=if_exists, index=index, method='multi')
