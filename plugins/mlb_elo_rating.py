"""MLB Elo Rating System for predicting game outcomes."""
...
class MLBEloRating:
    def __init__(self, k_factor=20, home_advantage=50, initial_rating=1500):
...
def load_mlb_games_from_db(db_path='data/nhlstats.duckdb'):
    """Load historical MLB games from DuckDB."""
    try:
        conn = duckdb.connect(str(Path(db_path)))
        
        # Check if MLB tables exist
        tables = conn.execute("SHOW TABLES").fetchall()
        table_names = [t[0] for t in tables]
        
        if 'mlb_games' not in table_names:
            print("⚠️ No mlb_games table found")
            return []
        ...
        print(f"✓ Loaded {len(games)} completed MLB games")
        return games
        
    except Exception as e:
        print(f"⚠️ Error loading MLB games: {e}")
        return []


def calculate_current_elo_ratings(output_path='data/mlb_current_elo_ratings.csv'):
    """Calculate current Elo ratings for all MLB teams."""
    games = load_mlb_games_from_db()
