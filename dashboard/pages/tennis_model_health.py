...
def render() -> None:
    ...
    st.caption(
        "Production-only governed holdout evidence comparing the tennis ensemble against calibrated Elo and the BetMGM market baseline."
    )
    ...
    verdict = (
        "beats BetMGM on shared holdout rows"
        if bool(latest["beats_betmgm"])
        else "does not yet beat BetMGM on shared holdout rows"
    )
    st.subheader(f"Latest tennis evaluation — {verdict}")
    st.caption(
        f"Source: {latest['data_source']} | Shared BetMGM rows: {int(latest['betmgm_holdout_rows'])}"
    )
    st.caption(
