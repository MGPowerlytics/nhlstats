...
def run_elo_simulation(games_df, league, k_factor, home_adv):
    ...
    for _, game in games_df.iterrows():
        # Predict
        if league == 'NCAAB':
            prob = elo.predict(game['home_team'], game['away_team'], is_neutral=game.get('is_neutral', False))
        elif league == 'WNCAAB':
            prob = elo.predict(game['home_team'], game['away_team'], is_neutral=game.get('neutral_site', False))
        else:
        ...
        if league == 'NBA':
            ...
            elo.update(game['home_team'], game['away_team'], game['home_win'], is_neutral=game.get('is_neutral', False))
        elif league == 'WNCAAB':
            elo.update(game['home_team'], game['away_team'], game['home_win'], is_neutral=game.get('neutral_site', False))
        elif league == 'EPL':
             elo.update(game['home_team'], game['away_team'], 'H' if game['home_win'] == 1 else 'A')
        elif league == 'Ligue1':
             elo.update(game['home_team'], game['away_team'], 'H' if game['home_win'] == 1 else 'A')
        elif league == 'Tennis':
...
def calculate_range_roi(df, ranges=[(90, 10), (80, 20), (70, 30), (60, 40), (50, 50)]):
    """Calculate ROI for different probability ranges."""
    if df.empty or 'elo_prob' not in df.columns:
        return pd.DataFrame()
    ...
    for top_pct, bottom_pct in ranges:
        # Get top X% of predictions
        top_threshold = np.percentile(df['elo_prob'], 100 - top_pct)
        top_subset = df[df['elo_prob'] >= top_threshold]

        # Get bottom Y% of predictions
        bottom_threshold = np.percentile(df['elo_prob'], bottom_pct)
        bottom_subset = df[df['elo_prob'] <= bottom_threshold]

        # Combine for range analysis
        range_subset = pd.concat([top_subset, bottom_subset])

        if len(range_subset) == 0:
            continue

        games = len(range_subset)
        wins = range_subset['home_win'].sum()
        win_rate = wins / games if games > 0 else 0
        avg_prob = range_subset['elo_prob'].mean()

        # Kalshi ROI calculation
        kalshi_price = avg_prob
        kalshi_roi = ((win_rate / kalshi_price) - 1) * 100 if kalshi_price > 0 else 0

        results.append({
            'Range': f'{top_pct}-{bottom_pct}',
            'Games': games,
            'Wins': wins,
            'Win Rate': win_rate,
            'Avg Prob': avg_prob,
            'ROI (Kalshi)': f"{kalshi_roi:.1f}%"
        })

    return pd.DataFrame(results)

def calculate_decile_probability_roi_matrix(df, kalshi_prices=[0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1]):
...
def betting_performance_page_v2():
    ...
    if not snapshots_df.empty:
        ...
        fig = px.line(plot_df, x="snapshot_hour_et", y="portfolio_value_dollars", title="Hourly Portfolio Value (ET)")
    ...
    with tab1:
        ...
        wl_data = pd.DataFrame({"Status": ["Won", "Lost", "Open"], "Count": [wins, losses, open_bets]})
        fig = px.pie(wl_data, values="Count", names="Status", title="Bet Outcomes",
                    color="Status", color_discrete_map={"Won": "green", "Lost": "red", "Open": "gray"})
        st.plotly_chart(fig, use_container_width=True)
    with tab2:
...
def financial_performance_page():
    ...
    # Daily P&L
    settled_bets['date'] = pd.to_datetime(settled_bets['placed_date']).dt.date
    daily_pl = settled_bets.groupby('date')['profit_dollars'].sum().reset_index()
    daily_pl['cumulative_pl'] = daily_pl['profit_dollars'].cumsum()

    # Weekly P&L
    settled_bets['week'] = pd.to_datetime(settled_bets['placed_date']).dt.to_period('W').astype(str)
    ...
    # Monthly P&L
    settled_bets['month'] = pd.to_datetime(settled_bets['placed_date']).dt.to_period('M').astype(str)
    ...
    # ROI by sport
    sport_pl = settled_bets.groupby('sport').agg({
        'profit_dollars': 'sum',
        'cost_dollars': 'sum',
        'contracts': 'sum'
    }).reset_index()
    sport_pl['roi'] = (sport_pl['profit_dollars'] / sport_pl['cost_dollars']) * 100
    sport_pl['win_rate'] = settled_bets[settled_bets['status'] == 'won'].groupby('sport').size() / settled_bets.groupby('sport').size() * 100
    ...
    win_rate = len(settled_bets[settled_bets['status'] == 'won']) / len(settled_bets) * 100
    ...
    with tab1:
        fig = px.line(daily_pl, x='date', y='cumulative_pl', title="Daily Cumulative P&L")
        fig.add_bar(x=daily_pl['date'], y=daily_pl['profit_dollars'], name="Daily P&L")
        st.plotly_chart(fig, use_container_width=True)

    with tab2:
        fig = px.line(weekly_pl, x='week', y='cumulative_pl', title="Weekly Cumulative P&L")
        fig.add_bar(x=weekly_pl['week'], y=weekly_pl['profit_dollars'], name="Weekly P&L")
        st.plotly_chart(fig, use_container_width=True)

    with tab3:
        fig = px.line(monthly_pl, x='month', y='cumulative_pl', title="Monthly Cumulative P&L")
        fig.add_bar(x=monthly_pl['month'], y=monthly_pl['profit_dollars'], name="Monthly P&L")
        st.plotly_chart(fig, use_container_width=True)

    # ROI by Sport
    st.subheader("Performance by Sport")
    fig = px.bar(sport_pl, x='sport', y='roi', title="ROI by Sport (%)",
                color='roi', color_continuous_scale='RdYlGn')
    st.plotly_chart(fig, use_container_width=True)

    # Sport details table
    st.subheader("Sport Performance Details")
    sport_display = sport_pl[['sport', 'profit_dollars', 'cost_dollars', 'roi', 'win_rate', 'contracts']].copy()
    sport_display.columns = ['Sport', 'Profit ($)', 'Cost ($)', 'ROI (%)', 'Win Rate (%)', 'Total Contracts']
...
def data_quality_page():
    ...
    try:
        from data_validation import validate_nhl_data, validate_mlb_data, validate_nfl_data
    except ImportError:
    ...
    for sport, report in all_reports.items():
        ...
        warning_checks = sum(1 for check in checks if check.get('severity') == 'warning')
        ...
        summary_data.append({
            ...
            'Errors': error_checks,
            'Health Score': (passed_checks / total_checks * 100) if total_checks > 0 else 0
        })
    ...
    # Health score gauge
    fig = px.bar(summary_df, x='Sport', y='Health Score',
    ...
    for sport, report in all_reports.items():
        with st.expander(f"{sport.upper()} Validation Details"):
            ...
            if report.stats:
                st.write("**Statistics:**")
                stats_df = pd.DataFrame(list(report.stats.items()), columns=['Metric', 'Value'])
...
def clv_analysis_page():
    ...
    if clv_data.get('num_bets', 0) == 0:
        st.warning("No CLV data available. CLV data needs to be backfilled from historical betting lines.")
        return
    ...
    fig = px.bar(sport_clv_df, x='sport', y='avg_clv',
    ...
    try:
        ...
        if not raw_clv_df.empty:
            fig = px.histogram(raw_clv_df, x='clv', nbins=50,
    ...
    try:
        ...
        if not correlation_df.empty:
            fig = px.bar(correlation_df, x='clv_category', y='win_rate',
    ...
    try:
        ...
        if not time_df.empty:
            fig = px.line(time_df, x='month', y='avg_clv',
...
page = st.sidebar.radio("📍 Navigation", ["Elo Analysis", "Betting Performance", "Financial Performance", "Data Quality", "CLV Analysis"], index=0)
...
if page == "Betting Performance":
    ...
    if raw_data.empty:
        st.warning(f"No data found for {league}. Please ensure data is backfilled into unified_games.")
        st.stop()
    ...
    if league in ['EPL', 'Ligue1']: default_home = 60
    if league == 'Tennis': default_home = 0; default_k = 32
    if league in ['NCAAB', 'WNCAAB']: default_home = 100
    ...
    with tab1:
        ...
        if not decile_stats.empty:
            fig = px.bar(decile_stats, x='Decile', y='Lift', color='Lift', color_continuous_scale='RdYlGn')
    ...
    with tab3:
        ...
        if not decile_stats.empty:
            ...
            decile_stats_copy['ROI Value'] = decile_stats_copy['ROI (-110)'].str.rstrip('%').astype(float)
            fig = px.bar(decile_stats_copy, x='Decile', y='ROI Value', color='ROI Value', color_continuous_scale='RdYlGn')
    ...
    with tab5:
        ...
        if simulated_data['glicko2_prob'].notna().any():
            ...
            col2.metric("Glicko-2 Accuracy", f"{gl_acc:.1%}", delta=f"{(gl_acc - elo_acc)*100:.1f} pts")
    ...
    with tab6:
        ...
        if not decile_stats.empty:
            ...
            st.markdown("**Rows**: Elo probability deciles | **Columns**: Kalshi contract prices (home team win odds) | **Values**: ROI if betting at that price")
    ...
    with tab7:
        ...
        if not simulated_data.empty:
            ...
            for bin_val in sorted(viz_df['Progress Bin'].unique()):
                ...
                timing_stats.append({'Progress (%)': bin_val, 'Brier Score': brier, 'Games': len(subset)})
            timing_df = pd.DataFrame(timing_stats)
            if not timing_df.empty:
                fig = px.line(timing_df, x='Progress (%)', y='Brier Score', markers=True)
                st.plotly_chart(fig, use_container_width=True)
