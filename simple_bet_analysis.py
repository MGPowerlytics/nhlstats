#!/usr/bin/env python3
"""
Simple script to analyze bets since February 6, 2025.
"""
...
# Create a new DBManager with explicit connection string
...
def main():
    ...
    try:
        print("Analyzing bets placed since February 6, 2025...")

        # Get all bets since start date
        all_bets = get_bets_since_date('2025-02-06')

        if len(all_bets) == 0:
            print("No bets found since February 6, 2025")
            return 1
        ...
        print(f"\nPERFORMANCE BY SEGMENT (min 5 bets):")
        ...
        # Filter for segments with at least 5 bets
        segment_stats = segment_stats[segment_stats['bets'] >= 5]
...
def generate_markdown_report(all_bets, settled_bets, sport_stats, segment_stats):
    ...
    filename = f"actual_betting_summary_{report_date}.md"

    with open(filename, 'w') as f:
        f.write(f"# Actual Betting Analysis Summary\n\n")
        f.write(f"**Date**: {datetime.now().strftime('%Y-%m-%d')}  \n")
        f.write(f"**Analysis Period**: Since February 6, 2025  \n")
