...
# ============================================================================
# ODDS COMPARATOR EXTENDED TESTS
# ============================================================================

class TestOddsComparatorExtended:
    """Extended tests for odds_comparator.py."""
    ...
    def test_match_markets_with_data(self, comparator):
        """Test match_markets with mock data."""
        with patch.object(comparator, 'load_markets') as mock_load:
            mock_load.side_effect = [
                [{'ticker': 'NBA-LAL-BOS', 'home_team': 'Lakers', 'away_team': 'Celtics', 'yes_ask': 55, 'title': 'Lakers vs Celtics'}],
                [{'event_id': '123', 'home_team': 'Lakers', 'away_team': 'Celtics', 'home_prob': 0.55}],
                []
            ]
    ...
    def test_identify_arbitrage_opportunities(self, comparator):
        """Test arbitrage identification if exists."""
        if hasattr(comparator, 'identify_arbitrage'):
            result = comparator.identify_arbitrage([])
            assert result == [] or isinstance(result, list)
    
    def test_calculate_implied_probability(self, comparator):
        """Test implied probability calculation if exists."""
        if hasattr(comparator, 'calculate_implied_probability'):
            result = comparator.calculate_implied_probability(2.0)
            assert result == 0.5
