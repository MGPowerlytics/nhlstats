"""Provider RED tests for the MLB base Elo boundary.

These tests exercise the real :class:`MLBEloRating` producer. The locked
...
def _load_schema() -> dict[str, Any]:
    return json.loads(
        (SCHEMAS_ROOT / "mlb_elo_prediction_v1.json").read_text(encoding="utf-8")
    )


def _assemble_payload(elo: MLBEloRating) -> dict[str, Any]:
...
class TestMLBBaseEloProviderContract:
    """Provider-side guarantees against the real ``MLBEloRating`` outputs."""
    ...
    def test_real_predict_returns_probability_in_unit_interval(self) -> None:
        ...
        prob = elo.predict(MLB_HOME_TEAM, MLB_AWAY_TEAM)
    ...
    def test_real_get_rating_returns_strictly_positive_rating(self) -> None:
        ...
        assert float(elo.get_rating(MLB_HOME_TEAM)) > 0.0
        assert float(elo.get_rating(MLB_AWAY_TEAM)) > 0.0

    def test_real_update_persists_rating_change(self) -> None:
        elo = MLBEloRating(k_factor=4.0, home_advantage=20.0)
        before_home = float(elo.get_rating(MLB_HOME_TEAM))
        before_away = float(elo.get_rating(MLB_AWAY_TEAM))

        elo.update(MLB_HOME_TEAM, MLB_AWAY_TEAM, True)

        after_home = float(elo.get_rating(MLB_HOME_TEAM))
        after_away = float(elo.get_rating(MLB_AWAY_TEAM))
    ...
    def test_producer_emits_native_structured_prediction_payload(self) -> None:
        elo = build_trained_mlb_elo()

        payload = elo.predict_payload(MLB_HOME_TEAM, MLB_AWAY_TEAM)
        validate_contract_payload(payload, _load_schema())
