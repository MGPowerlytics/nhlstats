...
class TestKalshiTennisMapping(unittest.TestCase):
    ...
    def test_market_title_parsing(self):
        ...
        titles = [
            (
                "Will Stefanos Tsitsipas win the Machac vs Tsitsipas : Round Of 64 match?",
                "Stefanos Tsitsipas",
            ),
            (
                "Will Michele Ribecai win the Ribecai vs Blanch : Round Of 16 match?",
                "Michele Ribecai",
            ),
            (
                "Will Naomi Osaka win the Osaka vs Cirstea : Round Of 64 match?",
                "Naomi Osaka",
            ),
        ...
        for title, expected_winner in titles:
            ...
            self.assertIn(
                w_code,
                [p1_code, p2_code],
                f"Winner code {w_code} ({winner}) not in matchup codes [{p1_code}, {p2_code}] ({p1} vs {p2})",
            )
