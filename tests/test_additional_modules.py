...
class TestDBLoaderExtended:
    ...
    def test_insert_and_query(self, db_loader):
        ...
        # Create a test table and clear existing data
        db_loader.conn.execute(text("CREATE TABLE IF NOT EXISTS test (id INT, name VARCHAR)"))
        db_loader.conn.execute(text("DELETE FROM test WHERE id = 1"))
        db_loader.conn.execute(text("INSERT INTO test VALUES (1, 'test')"))
        result = db_loader.conn.execute(text("SELECT * FROM test WHERE id = 1")).fetchall()
        assert len(result) == 1
