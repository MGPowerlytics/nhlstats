...
def test_all_classes_inherit_from_base():
    ...
    for elo_class in ELO_CLASSES:
        assert issubclass(elo_class, BaseEloRating), f"{elo_class.__name__} does not inherit from BaseEloRating"
...
def test_all_classes_have_required_methods():
    ...
    for elo_class in ELO_CLASSES:
        ...
        for method_name in required_methods:
            assert hasattr(instance, method_name), f"{elo_class.__name__} missing {method_name}"
            method = getattr(instance, method_name)
            assert callable(method), f"{elo_class.__name__}.{method_name} is not callable"
