...
class TestNHLDatabaseLoaderConnect:
    ...
    def test_connect_creates_tables(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Verify tables were created (Postgres query)
...
class TestNHLDatabaseLoaderClose:
    ...
    def test_close_with_connection(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            loader.connect()
            loader.close()
            # With Postgres, _conn may persist but is closed
...
class TestLoadDate:
    ...
    def test_load_date_no_data(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with NHLDatabaseLoader(str(db_path)) as loader:
                loader.load_date("2024-01-15", data_dir=data_dir)
...
class TestDatabaseQueries:
    ...
    def test_execute_query(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with NHLDatabaseLoader(str(db_path)) as loader:
                result = loader.conn.execute(text("SELECT 1 as test")).fetchall()
                assert result[0][0] == 1
    ...
    def test_insert_and_select(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with NHLDatabaseLoader(str(db_path)) as loader:
                ...
                loader.conn.execute(text("DELETE FROM teams WHERE team_id = 1"))
                ...
                # Query back
                result = loader.conn.execute(
                    text("SELECT team_name FROM teams WHERE team_id = 1")
                ).fetchone()
