...
# ============================================================
# Comprehensive tests for odds_comparator.py (30% -> higher)
# ============================================================

class TestOddsComparatorComprehensive:
    """Comprehensive tests for OddsComparator module."""
    
    def test_odds_comparator_import(self):
        """Test OddsComparator can be imported."""
        from odds_comparator import OddsComparator
        
        assert OddsComparator is not None
    
    def test_odds_comparator_has_platforms(self):
        """Test OddsComparator platforms list."""
        from odds_comparator import OddsComparator
        
        comparator = OddsComparator()
        
        assert hasattr(comparator, 'platforms')
        assert len(comparator.platforms) > 0
    
    def test_arbitrage_calculation(self):
        """Test arbitrage opportunity calculation."""
        # If sum of implied probabilities < 1, there's arbitrage
        prob1 = 0.45  # +122 American
        prob2 = 0.50  # +100 American
        
        total_implied = prob1 + prob2
        
        has_arbitrage = total_implied < 1.0
        
        assert has_arbitrage == True
    
    def test_no_arbitrage(self):
        """Test no arbitrage when probabilities sum > 1."""
        prob1 = 0.52  # -108 American
        prob2 = 0.52  # -108 American
        
        total_implied = prob1 + prob2
        
        has_arbitrage = total_implied < 1.0
        
        assert has_arbitrage == False
    
    def test_arbitrage_profit_calculation(self):
        """Test arbitrage profit calculation."""
        # If we can bet on both sides with implied < 1
        prob1 = 0.45
        prob2 = 0.50
        total_implied = prob1 + prob2  # 0.95
        
        # Arbitrage profit = 1 - total_implied
        profit_margin = 1 - total_implied
        
        assert profit_margin == pytest.approx(0.05)
