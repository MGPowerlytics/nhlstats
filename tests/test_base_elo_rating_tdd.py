...
from plugins.elo import (
    NHLEloRating, NBAEloRating, MLBEloRating, NFLEloRating,
...
class TestBaseEloRatingInterface:
    ...
    def test_base_elo_has_abstract_methods(self):
        ...
        abstract_methods = []
        for name, method in inspect.getmembers(BaseEloRating, predicate=inspect.isfunction):
            if getattr(method, '__isabstractmethod__', False):
                abstract_methods.append(name)

        expected_methods = {'predict', 'update', 'get_rating', 'expected_score', 'get_all_ratings'}
    ...
    def test_base_elo_type_hints(self):
        ...
        methods_to_check = ['predict', 'update', 'get_rating', 'expected_score', 'get_all_ratings']
...
class TestSportSpecificEloCompatibility:
    ...
    @pytest.mark.parametrize("elo_class", [
        NHLEloRating, NBAEloRating, MLBEloRating, NFLEloRating,
        EPLEloRating, Ligue1EloRating, NCAABEloRating, WNCAABEloRating, TennisEloRating
    ])
