...
try:
    with NHLDatabaseLoader(db_path="data/nhlstats.duckdb") as loader:
        ...
        # Show date ranges
        if nhl_count > 0:
            nhl_min = loader.conn.execute("SELECT MIN(game_date) FROM games").fetchone()[0]
            nhl_max = loader.conn.execute("SELECT MAX(game_date) FROM games").fetchone()[0]
            print(f"\nNHL: {nhl_min} to {nhl_max}")

        if mlb_count > 0:
            mlb_min = loader.conn.execute("SELECT MIN(game_date) FROM mlb_games").fetchone()[0]
            mlb_max = loader.conn.execute("SELECT MAX(game_date) FROM mlb_games").fetchone()[0]
            print(f"MLB: {mlb_min} to {mlb_max}")

        if nfl_count > 0:
            nfl_min = loader.conn.execute("SELECT MIN(game_date) FROM nfl_games").fetchone()[0]
            nfl_max = loader.conn.execute("SELECT MAX(game_date) FROM nfl_games").fetchone()[0]
