...
def backtest_betting_strategy(
    start_season="2025", elo_threshold=0.62, min_edge=0.05, max_bet=5.0, use_kelly=False
):
    ...
    for game in games:
        game_date, home_abbrev, home_name, away_abbrev, away_name, home_won, season = (
            game
        )
            ...
            if elo_prob >= elo_threshold and edge >= min_edge:
                ...
                if use_kelly:
                    bet_size = calculate_kelly_bet(
                        elo_prob, market_prob, edge, bankroll
                    )
    ...
    for bet in bets[-10:]:
        ...
        print(
            f"{bet['date']} {matchup:<20} {bet['elo_prob']:>5.1%} {bet['market_prob']:>5.1%} "
    ...
    for label, (min_edge, max_edge) in edge_buckets.items():
        ...
        if bucket_bets:
            ...
            bucket_roi = (
                (bucket_profit / bucket_wagered * 100) if bucket_wagered > 0 else 0
            )

            print(
                f"{label:<12} {len(bucket_bets):>6} {bucket_win_rate:>6.1%} "
