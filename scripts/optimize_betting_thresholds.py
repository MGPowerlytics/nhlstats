...
class ThresholdOptimizer:
    ...
    def generate_predictions(self) -> pd.DataFrame:
        ...
        df["decile"] = pd.qcut(
            df["elo_prob"], q=10, labels=range(1, 11), duplicates="drop"
        )
    ...
    def backtest_thresholds(
        ...
        print(
            f"  Backtesting {len(prob_thresholds)} prob × {len(edge_thresholds)} edge thresholds..."
        )
    ...
    def _simulate_bets(
        self, prob_threshold: float, edge_threshold: float, market_noise: float
    ) -> List[Dict]:
        ...
        for _, row in self.predictions.iterrows():
            ...
            row["home_win"]  # In hindsight, we know the true probability
            market_prob = np.clip(
                elo_prob + np.random.normal(0, market_noise), 0.01, 0.99
            )
...
def load_sport_data(sport: str) -> Tuple[object, pd.DataFrame]:
    ...
    if sport == "nba":
        ...
        games_df["home_win"] = (games_df["home_score"] > games_df["away_score"]).astype(
            int
        )
...
def analyze_sport(sport: str) -> Dict:
    ...
    print(
        lift_gain[
            ["decile", "num_games", "win_rate", "avg_elo_prob", "lift"]
    ...
    with open(output_dir / f"threshold_optimization_{timestamp}.json", "w") as f:
        ...
        for sport, data in results.items():
            serializable_results[sport] = {
                ...
                "lift_gain_summary": {
                    "high_decile_lift": float(
                        data["lift_gain"][data["lift_gain"]["decile"] >= 8][
                            "lift"
                        ].mean()
                    ),
                    "low_decile_lift": float(
                        data["lift_gain"][data["lift_gain"]["decile"] <= 3][
                            "lift"
                        ].mean()
                    ),
                },
