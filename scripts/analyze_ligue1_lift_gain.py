...
def analyze_ligue1(season=None):
    ...
    """
    print(f"\n{'=' * 80}")
    print("🇫🇷 LIGUE1 ELO LIFT/GAIN ANALYSIS")
    if season:
        print(f"Season: {season}/{season + 1}")
    else:
        print("All Historical Data (2021-2026)")
    print(f"{'=' * 80}\n")
    ...
    # Calculate deciles
    pred_df["decile"] = (
        pd.qcut(pred_df["elo_prob"], q=10, labels=False, duplicates="drop") + 1
    )
    ...
    pred_df_sorted["pct_wins_captured"] = (
        pred_df_sorted["cumulative_wins"] / total_wins * 100
    )
    pred_df_sorted["pct_games"] = range(1, len(pred_df_sorted) + 1)
    pred_df_sorted["pct_games"] = (
        pred_df_sorted["pct_games"] / len(pred_df_sorted) * 100
    )
    ...
    print(f"{'=' * 80}")
    print(
        f"{'Decile':<8} {'Games':<8} {'Avg Prob':<12} {'Home Wins':<12} {'Win Rate':<12} {'Lift':<8}"
    )
    print(f"{'=' * 80}")

    for _, row in decile_stats.iterrows():
        print(
            f"{int(row['decile']):<8} {int(row['games']):<8} "
    ...
    print(f"{'=' * 80}")
    print(
        f"Overall: {len(pred_df)} games, {pred_df['home_won'].sum()} home wins ({baseline_win_rate:.1%} rate)"
    )
    ...
    print(
        f"   {int(top_decile['games'])} games @ {top_decile['avg_prob']:.1%} avg probability"
    )
    ...
    print(
        f"   Home Wins (H): {result_dist.get('H', 0)} ({result_dist.get('H', 0) / len(pred_df) * 100:.1f}%)"
    )
    print(
        f"   Draws (D):     {result_dist.get('D', 0)} ({result_dist.get('D', 0) / len(pred_df) * 100:.1f}%)"
    )
    print(
        f"   Away Wins (A): {result_dist.get('A', 0)} ({result_dist.get('A', 0) / len(pred_df) * 100:.1f}%)"
    )
