...
def run_simulation(df, league):
    ...
    for _, row in df.iterrows():
        if league == "NCAAB":
            prob = elo.predict(
                row["home_team"], row["away_team"], is_neutral=row["is_neutral"]
            )
        else:
...
def analyze_season_timing():
    ...
    print(
        res_df.pivot(
            index="Season_Bin", columns="League", values="ROI_Home_Favs"
    ...
    print(
        res_df.pivot(
            index="Season_Bin", columns="League", values="Brier_Score"
