...
def evaluate_system(system_name, rating_system):
    ...
    results = {
        ...
        "all_accuracy": np.mean(
            [1 if (p > 0.5) == a else 0 for p, a in zip(all_preds, all_actuals)]
        ),
        "all_brier": np.mean([(p - a) ** 2 for p, a in zip(all_preds, all_actuals)]),
        "season_2025_games": len(season_2025_preds),
        "season_2025_accuracy": np.mean(
            [
                1 if (p > 0.5) == a else 0
                for p, a in zip(season_2025_preds, season_2025_actuals)
            ]
        ),
        "season_2025_brier": np.mean(
            [(p - a) ** 2 for p, a in zip(season_2025_preds, season_2025_actuals)]
        ),
        "season_2025_lift_gain": calculate_lift_gain(
            season_2025_preds, season_2025_actuals
        ),
...
def main():
    ...
    for label, key, better in metrics:
        ...
        if better == "higher":
            winner = (
                "Elo"
                if elo_val > glicko_val
                else "Glicko-2"
                if glicko_val > elo_val
                else "Tie"
            )
        else:
            winner = (
    ...
    for system_name, results in [("ELO", elo_results), ("GLICKO-2", glicko_results)]:
        ...
        print(
            f"{'Decile':<8} {'Games':>7} {'Wins':>6} {'WinRate':>8} {'AvgProb':>8} {'Lift':>6} {'Gain%':>7} {'Cover%':>7}"
        )
        print("-" * 80)

        for _, row in df.iterrows():
            print(
                f"{int(row['decile']):<8} {int(row['games']):>7} {int(row['wins']):>6} "
    ...
    for i in range(len(elo_lg)):
        ...
        winner = (
            "Elo"
            if elo_row["lift"] > glicko_row["lift"]
            else "Glicko-2"
            if glicko_row["lift"] > elo_row["lift"]
            else "Tie"
        )

        print(
            f"{int(elo_row['decile']):<8} {elo_row['lift']:>10.2f} {glicko_row['lift']:>12.2f} "
    ...
    elo_wins = sum(
        1
        for i in range(len(elo_lg))
        if elo_lg.iloc[i]["lift"] > glicko_lg.iloc[i]["lift"]
    )
    glicko_wins = sum(
        1
        for i in range(len(glicko_lg))
        if glicko_lg.iloc[i]["lift"] > elo_lg.iloc[i]["lift"]
    )
    ...
    if elo_results["season_2025_accuracy"] > glicko_results["season_2025_accuracy"]:
        print("✅ ELO")
        print(
            f"  Advantage: +{(elo_results['season_2025_accuracy'] - glicko_results['season_2025_accuracy']) * 100:.2f}% accuracy"
        )
    elif glicko_results["season_2025_accuracy"] > elo_results["season_2025_accuracy"]:
        print("✅ GLICKO-2")
        print(
            f"  Advantage: +{(glicko_results['season_2025_accuracy'] - elo_results['season_2025_accuracy']) * 100:.2f}% accuracy"
        )
    else:
        print("🤝 TIE")
