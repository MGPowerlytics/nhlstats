...
def find_value():
    ...
    for km in kalshi_markets:
        ...
        for em in all_external_odds:
            ...
            if (norm_p1 == em_home and norm_p2 == em_away) or (norm_p1 == em_away and norm_p2 == em_home):
                ext_match = em
                break
        ...
        if ext_match:
            ...
            pin = ext_match['bookmakers'].get('pinnacle') or ext_match['bookmakers'].get('betmgm') or list(ext_match['bookmakers'].values())[0]
            if pin:
        ...
        if edge > 0.03: # 3% edge
            ...
            print(f"{matchup_str[:40]:<40} | {kalshi_prob:6.1%} | {sharp_str:7} | {elo_prob:6.1%} | {edge:+.1%}")
