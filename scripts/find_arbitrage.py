...
class ArbitrageFinder:
    ...
    def calculate_win_probability(
        self, home_rating: float, away_rating: float, home_advantage: float = 100
    ) -> float:
    ...
    def find_opportunities(self, sport: str, date_str: str) -> List[Dict]:
        ...
        for game in sportsbook_odds:
            ...
            elo_prob = self.calculate_win_probability(
            ...
            if kalshi_match:
                ...
                elo_prob - kalshi_prob
                elo_prob - best_home_prob
        ...
        opportunities.sort(
            key=lambda x: x.get("edge", x.get("profit_margin", 0)), reverse=True
        )
    ...
    def _find_kalshi_match(
        self, home_team: str, away_team: str, kalshi_markets: List[Dict]
    ) -> Dict:
    ...
    def print_opportunities(self, opportunities: List[Dict]):
        ...
        if "value_bet" in by_type:
            ...
            for opp in by_type["value_bet"][:20]:
                side_team = (
                    opp["home_team"] if opp["side"] == "home" else opp["away_team"]
                )
                print(f"\n{opp['away_team']} @ {opp['home_team']}")
                print(f"  Bet {opp['side'].upper()}: {side_team}")
                print(f"  Platform: {opp['platform']}")
                print(
                    f"  Edge: {opp['edge']:.1%} (Elo: {opp['elo_prob']:.1%}, Market: {opp['market_prob']:.1%})"
                )
