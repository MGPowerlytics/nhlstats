...
def analyze_value():
    ...
    df = df.dropna(
        subset=["B365W", "B365L", "PSW", "PSL", "AvgW", "AvgL", "MaxW", "MaxL"]
    )
    df = df[
        (df["B365W"] > 0)
        & (df["B365L"] > 0)
    ...
    df["market_prob_winner"] = df.apply(
        lambda x: remove_vig(x["AvgW"], x["AvgL"]), axis=1
    )
    ...
    for name, w_col, l_col in strategies:
        ...
        if name == "Kalshi Mispricing vs Sharp (Pinnacle)":
            ...
            df["market_vs_sharp_edge"] = (
                df["sharp_prob_winner"] - df["market_prob_winner"]
            )
            bets_win = df[
                df["market_vs_sharp_edge"] > 0.03
            ].copy()  # 3% market discrepancy
    ...
    print("\n💡 CONCLUSION:")
    print(
        "  1. Searching multiple bookmakers (Max Price) significantly improves ROI by capturing price laggards."
    )
    print(
        "  2. Using Pinnacle (Sharp) as a secondary filter reduces bet volume but increases win rate and reliability."
    )
    print(
        "  3. For Kalshi, you should look for athletes where Kalshi price is 'off' relative to BOTH Elo AND sharp bookies like BetMGM/Pinnacle."
    )
