...
def analyze_strategy_impact(excluded_segments):
    ...
    for sport, confidence in excluded_segments:
        mask = (filtered_bets['sport'] == sport) & (filtered_bets['confidence'] == confidence)
...
def print_report(report):
    ...
    if not report['worst_segments'].empty:
        ...
        for _, row in report['worst_segments'].head(5).iterrows():
            print(f"{row['sport']} {row['confidence']}: {row['total_bets']} bets, "
...
def main():
    ...
    try:
        ...
        print(f"✓ Database connection successful. Bets in database: {test_result.iloc[0][0]}")
