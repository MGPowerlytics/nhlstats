...
def analyze_soccer_sharp_value(sport_code, league_name):
    ...
    for f in files:
        try:
            ...
            required = ['HomeTeam', 'AwayTeam', 'FTR', 'Date', 'PSH', 'PSD', 'PSA', 'AvgH', 'AvgD', 'AvgA']
    ...
    market_probs = df.apply(lambda x: remove_vig_3way(x['AvgH'], x['AvgD'], x['AvgA']), axis=1)
    ...
    sharp_probs = df.apply(lambda x: remove_vig_3way(x['PSH'], x['PSD'], x['PSA']), axis=1)
    ...
    for name, mode in strategies:
        ...
        for _, row in df.iterrows():
            outcomes = [
                ('H', row['elo_prob_H'], row['market_prob_H'], row['sharp_prob_H'], row['AvgH']),
                ('D', row['elo_prob_D'], row['market_prob_D'], row['sharp_prob_D'], row['AvgD']),
                ('A', row['elo_prob_A'], row['market_prob_A'], row['sharp_prob_A'], row['AvgA'])
            ]

            for side, elo_p, mark_p, sharp_p, odds in outcomes:
                bet = False
                if mode == "elo_only":
                    if elo_p - mark_p > threshold: bet = True
                elif mode == "elo_sharp":
                    if (elo_p - mark_p > threshold) and (sharp_p > mark_p + 0.01): bet = True
                elif mode == "sharp_value":
                    if sharp_p - mark_p > 0.01: bet = True
