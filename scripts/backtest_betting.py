...
class BettingBacktest:
    ...
    def analyze_game(self, game: Dict, sport: str) -> Optional[Dict]:
        ...
        try:
            ...
            home_score = next((s['score'] for s in scores if s['name'] == home_team_name), None)
            away_score = next((s['score'] for s in scores if s['name'] == away_team_name), None)
            ...
            result = {
                ...
                'elo_correct': (elo_prob > 0.5) == home_won if elo_prob and not is_draw else None,
                'glicko2_correct': (glicko2_prob > 0.5) == home_won if glicko2_prob else None,
            }
    ...
    def run_backtest(self, sports: List[str], days: int = 3):
        ...
        if len(elo_df) > 0:
            ...
            elo_brier = ((elo_df['elo_prob'] - elo_df['home_won'].astype(float)) ** 2).mean()
            print(f"\n🎲 ELO RATING SYSTEM:")
            ...
            if len(high_conf) > 0:
                ...
                print(f"  High Confidence (>70%): {len(high_conf)} games, {high_accuracy:.1%} accuracy")

        # Glicko-2 performance
        glicko2_df = df[df['glicko2_prob'].notna()]
        if len(glicko2_df) > 0:
            glicko2_accuracy = glicko2_df['glicko2_correct'].mean()
            glicko2_brier = ((glicko2_df['glicko2_prob'] - glicko2_df['home_won'].astype(float)) ** 2).mean()
        ...
        for sport in df['sport'].unique():
            ...
            if len(sport_glicko2) > 0:
                print(f"  Glicko-2 Accuracy: {sport_glicko2['glicko2_correct'].mean():.1%}")

        # Show some examples
        print(f"\n{'='*80}")
        print(f"🎮 RECENT PREDICTIONS (Sample)")
        print(f"{'='*80}")

        sample_df = df.head(10)
        for _, row in sample_df.iterrows():
            winner = row['home_team'] if row['home_won'] else row['away_team']
            elo_str = f"Elo:{row['elo_prob']:.1%}" if pd.notna(row['elo_prob']) else "Elo:N/A"
            g2_str = f"G2:{row['glicko2_prob']:.1%}" if pd.notna(row['glicko2_prob']) else "G2:N/A"

            elo_mark = "✓" if row['elo_correct'] else "✗" if pd.notna(row['elo_correct']) else "-"
            g2_mark = "✓" if row['glicko2_correct'] else "✗" if pd.notna(row['glicko2_correct']) else "-"

            print(f"{row['sport']:6} | {row['home_team'][:15]:15} vs {row['away_team'][:15]:15} | "
