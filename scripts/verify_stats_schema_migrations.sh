#!/usr/bin/env bash
# Verify the governed stats-schema migration chain and ledger state.
...
echo "=== verify_stats_schema_migrations.sh ==="
echo "Repo: ${REPO_ROOT}"
echo ""

run_locally() {
    echo "▶ Verifying migrations from host via localhost PostgreSQL"
    (
        cd "${REPO_ROOT}"
        POSTGRES_HOST=localhost PYTHONPATH="${REPO_ROOT}" \
            python -m plugins.schema_migrations verify \
            --migration-dir "${LOCAL_MIGRATION_DIR}"
    )
}

run_in_compose() {
    echo "▶ Verifying migrations from airflow-scheduler container"
    docker compose exec -T airflow-scheduler bash -lc \
        "cd /opt/airflow && python -m plugins.schema_migrations verify --migration-dir ${CONTAINER_MIGRATION_DIR}"
}
...
echo ""
echo "✓ verify_stats_schema_migrations.sh finished successfully."
