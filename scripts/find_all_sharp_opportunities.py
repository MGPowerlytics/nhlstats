...
def find_volume_opportunities():
    ...
    configs = {
        "tennis": {
            "fetch": fetch_tennis_markets,
            "system": TennisEloRating(),
            "threshold": 0.60,
        },
        "nhl": {
            "fetch": fetch_nhl_markets,
            "system": NHLEloRating(),
            "threshold": 0.66,
        },
        "ligue1": {
            "fetch": fetch_ligue1_markets,
            "system": Ligue1EloRating(),
            "threshold": 0.45,
        },
    }
    ...
    for sport, cfg in configs.items():
        ...
        if sport == "tennis":
            ...
            elo_data = {
                "ATP": dict(zip(atp_df["team"], atp_df["rating"])),
    ...
    print("\n" + "=" * 120)
    print(
        f"{'SPORT':<8} | {'MATCHUP':<40} | {'BET ON':<15} | {'KALSHI':<7} | {'ELO':<7} | {'EDGE'} | {'SHARP'}"
    )
    print("-" * 120)
    ...
    if not all_opps:
        ...
        for o in sorted(all_opps, key=lambda x: x["edge"], reverse=True):
            ...
            print(
                f"{o['sport'].upper():<8} | {o['away_team'][:18]} @ {o['home_team'][:18]:<19} | {o['bet_on'][:15]:<15} | {o['market_prob']:6.1%} | {o['elo_prob']:6.1%} | {o['edge']:+.1%} | {sharp_status}"
            )
