...
def main():
    ...
    print(f"\n{'=' * 80}")
    print("📊 COMPREHENSIVE BETTING PERFORMANCE ANALYSIS")
    print(f"{'=' * 80}\n")
    ...
    for label, low, high in confidence_bins:
        ...
        if len(subset) > 0:
            ...
            print(
                f"  {label:20} | Games: {len(subset):2} | Accuracy: {acc:5.1%} | Avg Prob: {avg_prob:.1%}"
            )
    ...
    for sport in sorted(elo_df["sport"].unique()):
        ...
        brier_sport = (
            (sport_df["elo_prob"] - sport_df["home_won"].astype(float)) ** 2
        ).mean()
    ...
    print(f"{'=' * 80}")
    print("\nAssuming:")
    print("  - $100 flat bet per game")
    print("  - American odds conversion")
    print("  - Only bet when Elo confidence > 65%")
    print("  - Hypothetical market odds match implied probabilities\n")
    ...
    betting_df["profit"] = betting_df.apply(
        lambda row: calculate_profit(row["elo_correct"], row["elo_prob"]), axis=1
    )
    ...
    for _, row in best.iterrows():
        ...
        print(
            f"  {result} {row['sport']:6} | {row['home_team'][:15]:15} @ {row['elo_prob']:.1%} | "
    ...
    if len(high_edge_df) > 0:
        ...
        if kelly_fraction > 0:
            print("  ✅ Positive expected value - Kelly says BET!")
        else:
            print("  ⚠️ Negative expected value - Kelly says PASS")
    ...
    nhl_acc = (
        elo_df[elo_df["sport"] == "NHL"]["elo_correct"].mean()
        if "NHL" in elo_df["sport"].values
        else 0
    )
    ...
    print(
        f"4. Overall ROI of {roi:+.1f}% suggests system has {'+EDGE' if roi > 0 else 'NO EDGE'}"
    )
    ...
    print("\n💡 RECOMMENDATIONS:")
    if nba_acc > 0.70:
        print("  ✓ Focus on NBA - showing strong edge")
    if nhl_acc < 0.50:
        print("  ⚠️ Avoid NHL for now - below 50% accuracy")
    print("  ✓ Only bet high confidence games (>70%)")
    print("  ✓ Use Kelly Criterion for position sizing")
    print("  ✓ Track at least 100 bets before drawing final conclusions")
